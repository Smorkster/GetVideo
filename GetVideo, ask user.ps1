<#
.SYNOPSIS
	Same as GetYouTube, but this version checks with the user if the video is to be downloaded
#>
...
for(; $start -lt $links.Count; $start++)
{
	...
	if(($comp.FreeSpace / 1GB) -gt (1.5))
	{
		...
		if($ans -match "y")
		{
			...
			& .\youtube-dl.exe $video -o $videoname -u "thelampan@gmail.com" -p "ollonborretl3" --mark-watched --no-playlist -q
			$ErrorActionPreference = "Continue"
			if($Error[0] -ne "")
			{
				Write-Host "Error while downloading" -Foreground Red
				if((get-childitem | ? {$_.Name -match "Bellular "+$index}).Length -eq 0)
				{
