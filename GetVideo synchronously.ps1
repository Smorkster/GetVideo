...

{
...

	{
		...
		[System.Collections.ArrayList] $parameters = @('https://www.youtube.com/playlist?list=WL', '--cookies', 'cookies.txt', '--no-progress', '-o', '%(upload_date)s %(uploader)s %(title)s.%(ext)s', '--mark-watched')
		...
		.\youtube-dl.exe $parameters | % {
			...
			if ($_ -match "Downloading video \d of \d") {
